...
#преобразование текстовых цифр из немецкого типа разделения разрядов и долей
def float_de_de(str: str) -> float:
  return float(str.replace('.','').replace(',','.'))

#метод доступа к базе данны
...
class InvestingParse():
  ...
  #метод переходя по меню до главной таблицы TODO фабрика
  ...
  #получение url для перехода к детальным отчетам
